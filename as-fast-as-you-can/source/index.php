...
$hint = "Post decode([Get-flag]) as [admin] as fast as you can, then your will get flag";
header("Hint: $hint");
$get_flag = base64_encode($_SESSION['random_string']);
header("Get-flag: $get_flag");
if(isset($_POST['admin'])){
	if($_POST['admin'] === $_SESSION['random_string']){
		$cost = getMilliSecond() - $_SESSION['random_string_create_time'];
		if ($cost < 3000){
			echo "Wow, prefect! Here is the flag : LAB_ATMNC{d146f2af-d403-42ec-aa50-0de36c073796}";
		}else{
		...
		echo '<input name="admin" type="text">';
